    ...
    <el-row class="lives-news">
        ...
        <el-row>
            <el-table :data="tagLists" class="table">
                <el-table-column prop="goodsAppraiseTagId" label="标签ID" align="center"></el-table-column>
                
                <el-table-column prop="goodsAppraiseTagName" label="标签名称" align="center"></el-table-column>
        ...
        <el-dialog :title="isAdd?'新增评价标签':'修改评价标签'" :visible.sync="addDialog" size="tiny" @close="closeaddDialog" class="dialog">
            <el-form :model="tagObj" label-width="120px">
                <el-form-item label="标签名称">
                    <el-input v-model="tagObj.goodsAppraiseTagName" :maxlength="6"></el-input>
                </el-form-item>
            </el-form>
...
<script>
import {
    getGoodsAppraiseTagLists,
    addGoodsAppraiseTag,
    deleteGoodsAppraiseTag,
    updateGoodsAppraiseTagById
} from '@/api/api'
export default {
    data: function() {
        return {
    ...
    methods: {
        ...
        getTagLists: function() {
            getGoodsAppraiseTagLists({ params: { pageId: this.pageId, pageSize: this.pageSize } }).then(data => {
                console.log(data)
        ...
        addCategory: function() {
            ...
            if (this.isAdd) {
                addGoodsAppraiseTag(this.tagObj).then(data => {
                    this.getTagLists();
                    this.$message({
                        message: '添加成功',
                        type: 'success'
                ...
                updateGoodsAppraiseTagById(this.tagObj).then(data => {
                    this.getTagLists()
                    this.$message({
                        message: '修改成功',
                        type: 'success'
        ...
        updateTag: function(index, row) {
            ...
            this.addDialog = true;
            this.tagObj = {
                goodsAppraiseTagId: row.goodsAppraiseTagId,
                goodsAppraiseTagName: row.goodsAppraiseTagName
            }
        },
        //通过id删除分类
        deleteTag: function(index, row) {
            this.$confirm('是否删除该标签?', '删除标签', {
                ...
                deleteGoodsAppraiseTag(row.goodsAppraiseTagId).then(() => {
                    this.getTagLists();
                    this.$message({
                        type: 'success',
                        message: '删除成功!'
